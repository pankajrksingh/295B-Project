...
def get_smple_rate(dataset, time_type):
    ...
    if time_type == 'timer':
        ...
        measures['fs'] = math.ceil((float(len(sampletimer)) / float(sampletimer[-1])) * 1000)
...
def filter_signal(dataset, fs):
    ...
    filtered = butter_lowpass_filter(dataset.hart, 2.0, fs, 5)
    ...
    a = 25.0
    while a>0:
        filtered = butter_lowpass_filter(dataset.hart, a, fs, 5)
        plt.subplot(211)
        plt.plot(dataset.hart, color='Blue', alpha=0.5, label='Original Signal')
        plt.legend(loc=4)

        plt.subplot(212)
        plt.plot(filtered, color='Red', label='Filtered Signal - %f'%a)
        # plt.ylim(200,
